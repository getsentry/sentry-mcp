  ...
  async handler(params, context: ServerContext) {
    ...
    if (params.attachmentId) {
      ...
      const contentParts: (TextContent | ImageContent | EmbeddedResource)[] =
        [];
      // Use the Content-Type from the download response (Step 2) rather than
      // the mimetype field from the metadata endpoint (Step 1). The two can
      // disagree — notably the SDK may upload attachments as octet-stream even
      // when the file is an image — and the download response is authoritative.
