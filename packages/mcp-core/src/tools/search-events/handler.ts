...
function buildSearchRepairPrompt(params: {
  ...
  return [
    ...
    "For spans, logs, and metrics, look up explicit fields or query filters with datasetAttributes substringMatch before dropping or renaming them.",
    "A broad datasetAttributes result may be truncated, so absence from a broad preview does not prove an explicit field is invalid — confirm with a targeted substringMatch lookup.",
