...
type AutofixStatus = z.infer<typeof AutofixStatusSchema>;
...
export interface AutofixSection {
  ...
  mergedFilePatches: AutofixFilePatch[];
}
...
export function getOrderedAutofixSections(
  ...
  const mergedByFile = new Map<string, AutofixFilePatch>();
...
function renderFilePatchesArtifact(patches: AutofixFilePatch[]): string {
...
function renderPullRequestsArtifact(
  pullRequests: AutofixRepoPRState[],
): string {
