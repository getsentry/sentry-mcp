...
export const TOOL_HANDLERS = {
  ...
  get_event_attachment: async (context, params) => {
    ...
    if (!params.projectSlug) {
      throw new UserInputError("Project slug is required");
    }
    ...
    try {
      ...
      if (params.attachmentId) {
        ...
        const contentParts: (EmbeddedResource | TextContent)[] = [];
        const isText = attachment.attachment.mimetype?.startsWith("text/");
        ...
        // If it's a text file and we have blob content, decode and display it
        if (isText) {
          ...
          output += `The content of this attachment is binary and cannot be displayed in this tool.\n`;
          output += `It has been returned as an embedded resource and may be accessed through your agent.\n`;
        }
      ...
      attachments.forEach((attachment, index) => {
        ...
        output += `To download this attachment, use the "get_event_attachment" tool:\n`;
      ...
      if (error instanceof ApiError) {
        if (error.status === 404) {
          return `# Event Not Found\n\nNo event found with ID: ${params.eventId} in project ${params.projectSlug}`;
        }
        throw error;
      }
      throw error;
