...
export default defineTool({
  name: "search_events",
  description: [
    "Search for error events, log entries, or trace spans with both individual events and SQL-like aggregations.",
    "",
    "QUERY MODES:",
    "1. Individual Events - Raw event data (default when no functions used)",
    "2. Aggregate Queries - SQL-like groups when using count(), avg(), etc.",
    "",
    "AGGREGATE FUNCTIONS:",
    ...
    "",
    "USE FOR:",
    "- 'error logs from last hour' → Individual events",
    "- 'count errors by type' → Aggregate with count()",
    "- 'slowest API calls' → Individual spans",
    "- 'average response time' → Aggregate with avg()",
    "",
    "❌ DO NOT USE for 'issues' (use find_issues)",
    "",
    "DATASET SELECTION:",
    "- 'error logs' → dataset='logs'",
    "- 'exceptions/crashes' → dataset='errors'",
    "- 'slow queries/performance' → dataset='spans'",
    "",
    "<examples>",
    "### Error logs",
    ...
    "",
    "<hints>",
    "- Be specific: time ranges, error types, thresholds",
    "- 'error logs' means dataset='logs' NOT 'errors'",
    "</hints>",
  ].join("\n"),
  inputSchema: {
