...
export default function Ghost() {
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      const skew = Math.min(Math.max(-distX * 0.1, -50), 50);
      const scaleY = Math.min(Math.max(1 - distY / 250, 0.5), 1.5);
      const blur = Math.min(
        Math.max(Math.abs(distX / 30) + Math.abs(distY / 30), 0),
        2,
      );

      // apply everything in one transform for best performance
      ghost.style.transform = `
        translateX(${ghostX}px)
        translateY(${ghostY}px)
        skew(${skew}deg)
        scale(1, ${scaleY})
      `;
  ...
  return (
    // biome-ignore lint/a11y/noSvgWithoutTitle: <it's decorative>
    <svg
      id="cursorthing"
