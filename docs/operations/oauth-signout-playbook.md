...
- `app.client.registration.method` — `cimd` | `dcr` | `unknown`, classified from the OAuth `client_id` (HTTPS URL => CIMD; opaque id => DCR)
- `user.id`
...
- `app.client.registration.method` — always `dcr` on this endpoint
