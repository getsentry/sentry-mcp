...
// Function to find a platform by its name (case-insensitive) or id
export function findPlatformByNameOrId(
  nameOrId: string,
): PlatformIntegration | undefined {
  const normalizedSearch = nameOrId.toLowerCase().trim().replace(/\./g, "");

  // Search through platforms for a match
  return platforms.find(
    (p) =>
      p.id === normalizedSearch ||
      p.name.toLowerCase().replace(/\./g, "").includes(normalizedSearch),
  );
}
